...
# 検査員列の表示上限（UI/出力の列数制約）
MAX_INSPECTORS_PER_LOT = 10
# 追加：勤務時間の10%超過を許容
WORK_HOURS_OVERRUN_RATE = 0.1
SAME_DAY_WORK_HOURS_OVERRUN_RATE = 0.1
...
class InspectorAssignmentManager:
    ...
    def __init__(
        ...
        # 検査員名の正規化インデックス（スペースや互換文字差異を吸収）
        self.inspector_name_to_row_normalized = {}
    ...
    def _is_locked_fixed_preinspection_lot(
        ...
        try:
            ...
            for i in range(1, MAX_INSPECTORS_PER_LOT + 1):
                ...
                if current_name and self._is_fixed_inspector_for_lot(product_number, process_name, current_name):
                    return True
            return False
        except Exception:
            return False
    ...
    def _should_force_keep_assignment(self, shipping_date: Any) -> bool:
        """Return False to allow unassignment when constraints are violated."""
        return False

    def _resolve_lot_date(self, shipping_date: Any, current_date: date) -> date:
        """Return date key for daily hours (workday-based)."""
        return current_date
    ...
    def _rebuild_assignment_histories(self, result_df: pd.DataFrame, inspector_master_df: pd.DataFrame) -> None:
        ...
        for row in result_df.itertuples(index=False):
            ...
            lot_date_for_rebuild = self._resolve_lot_date(shipping_date_raw, current_date)
            product_name_str = ''
            if '品名' in cols:
            ...
            for i in range(1, MAX_INSPECTORS_PER_LOT + 1):
                ...
                self.inspector_daily_assignments.setdefault(inspector_code, {}).setdefault(lot_date_for_rebuild, 0.0)
                self.inspector_daily_assignments[inspector_code][lot_date_for_rebuild] += divided_time
    ...
    def _calc_required_inspectors(self, inspection_time: float, threshold: Optional[float] = None) -> int:
        ...
        return min(MAX_INSPECTORS_PER_LOT, calculated)
    ...
    def _is_fixed_inspector_for_lot(
        ...
        if not fixed_names:
            return False
        normalized_fixed = {self._normalize_person_name(n) for n in fixed_names if self._normalize_person_name(n)}
        return self._normalize_person_name(inspector_name_str) in normalized_fixed
        ...
        for row_tuple in inspector_master_df.itertuples(index=True):
            ...
            if name_col_idx >= 0:
                ...
                if pd.notna(name) and str(name).strip():
                    ...
                    normalized_key = self._normalize_person_name(name_key)
                    if normalized_key and normalized_key not in self.inspector_name_to_row_normalized:
                        self.inspector_name_to_row_normalized[normalized_key] = row
    ...
    def _get_inspector_by_name(
        ...
        normalized_key = self._normalize_person_name(name_key)
        if normalized_key:
            normalized_row = self.inspector_name_to_row_normalized.get(normalized_key)
    ...
    def _calculate_feasibility_and_candidates(
        ...
        ship_date_col_idx = result_cols.get('出荷予定日', -1)
    ...
    def _sort_lots_by_priority(
        ...
        Returns:
            ソートされたDataFrame
        """
        # 出荷予定日の優先順位を設定（高優先度判定用）
        # 優先度: 1=当日, 2=当日洗浄/先行検査, 3=3営業日以内, 4=その他
        ...
        # 出荷予定日昇順＋指示日（FIFO）を最優先にソート
        # 候補数や新規品判定はFIFOの後に回す
        result_df['_shipping_sort_key'] = result_df['出荷予定日'].apply(self._normalize_shipping_date)
        result_df['_original_index'] = result_df.index
        result_df = result_df.sort_values(
            ['_shipping_sort_key', '_instruction_date_sort_key', '_sort_product_id', '_original_index'],
            ...
            for product_num in result_df['品番'].unique():
                ...
                for shipping_date in product_lots['出荷予定日'].unique():
                    ...
                    if len(same_product_same_shipping) > 1:
                        instruction_dates = same_product_same_shipping['_instruction_date_sort_key'].dropna()
        ...
        preview_count = min(10, len(result_df))
        if preview_count > 0:
            preview_dates = result_df['出荷予定日'].head(preview_count).apply(
                lambda x: str(x).strip() if pd.notna(x) else 'N/A'
            ).tolist()
            self.log_message(
                f"出荷予定日昇順の先頭{preview_count}件をプレビュー: {preview_dates}",
                level='info'
            )
        ...
        if not fixed_inspector_lots.empty:
            ...
            self.log_message(f"固定検査員が設定されている品番のロット数: {len(fixed_inspector_lots)}ロット（品番: {list(fixed_products)}）")

        self.log_message("並び順ロジック: 出荷予定日昇順 → 指示日（FIFO） → 品番 の順で並び替えました。")
    ...
    def assign_inspectors(
        ...
        Returns:
            割り当て結果を含むDataFrame（検査員1〜10列が追加される）
        """
        try:
            ...
            # 【高速化】同一実行内の参照をキャッシュ（結果は変えない）
            self._skill_rows_by_product_cache = {}
            self._new_team_inspectors_cache = None
            ...
            for row_idx, row in enumerate(result_df.itertuples(index=False)):
                ...
                fixed_inspector_names = self._collect_fixed_inspector_names(product_number, process_name)
                allow_multiple_fixed = inspection_time > self.product_limit_hard_threshold
                ...
                if fixed_inspector_names:
                    ...
                    other_inspectors = []
                    fixed_name_norms = {
                        self._normalize_person_name(n)
                        for n in fixed_inspector_names
                        if self._normalize_person_name(n)
                    }
                    available_inspector_names = {
                        self._normalize_person_name(insp.get('氏名', ''))
                        for insp in available_inspectors
                        if self._normalize_person_name(insp.get('氏名', ''))
                    }
                    
                    for inspector in available_inspectors:
                        inspector_name = inspector.get('氏名', '')
                        if self._normalize_person_name(inspector_name) in fixed_name_norms:
                            fixed_inspectors.append(inspector)
                        else:
                            other_inspectors.append(inspector)
                    
                    # 【追加】base_candidatesに含まれていない固定検査員を追加
                    missing_fixed_inspectors = [
                        name for name in fixed_inspector_names
                        if self._normalize_person_name(name) not in available_inspector_names
                    ]
                    if missing_fixed_inspectors:
                        ...
                        # 追加された固定検査員を確認
                        complete_inspector_names = {
                            self._normalize_person_name(insp.get('氏名', ''))
                            for insp in complete_candidates
                            if self._normalize_person_name(insp.get('氏名', ''))
                        }
                        for missing_name in missing_fixed_inspectors:
                            missing_norm = self._normalize_person_name(missing_name)
                            if missing_norm and missing_norm in complete_inspector_names:
                                # complete_candidatesから該当する検査員を取得
                                for insp in complete_candidates:
                                    if self._normalize_person_name(insp.get('氏名', '')) == missing_norm:
                                        fixed_inspectors.append(insp)
                                        available_inspector_names.add(missing_norm)
                        ...
                        for missing_name in missing_fixed_inspectors:
                            if self._normalize_person_name(missing_name) in available_inspector_names:
                                continue
                            ...
                            available_inspector_names.add(self._normalize_person_name(missing_name))
                    ...
                    if fixed_inspectors:
                        ...
                        # ただし長時間ロットでは複数固定検査員を許容し、過大割当を回避する
                        if force_fixed_assignment and fixed_primary_name and not allow_multiple_fixed:
                            fixed_inspectors = [
                ...
                # 固定検査員ロットの長時間分散（4時間上限の遵守に必要な人数を確保）
                if fixed_inspector_names and inspection_time > self.product_limit_hard_threshold:
                    required_by_product_limit = int(np.ceil(inspection_time / self.product_limit_hard_threshold))
                    max_fixed_available = len(fixed_inspector_names)
                    if max_fixed_available > 0:
                        required_inspectors = min(
                            max_fixed_available,
                            max(required_inspectors, required_by_product_limit),
                        )
                        self.log_message(
                            f"固定検査員ロット {product_number}: 4時間上限のため必要人数を {required_inspectors}人に調整しました",
                            level='info'
                        )

                # 固定検査員ロット（長時間）は固定検査員のみで割当てる
                if fixed_inspector_names and allow_multiple_fixed:
                    if fixed_inspectors and len(fixed_inspectors) >= required_inspectors:
                        available_inspectors = fixed_inspectors
                        self.log_message(
                            f"固定検査員ロット {product_number}: 固定検査員のみで割当を実行します（候補: {len(fixed_inspectors)}名, 必要: {required_inspectors}人）",
                            level='info'
                        )
                if required_inspectors > MAX_INSPECTORS_PER_LOT:
                    self.log_message(
                        f"必要人数が上限 {MAX_INSPECTORS_PER_LOT}人を超えるため、"
                        f"{MAX_INSPECTORS_PER_LOT}人に制限します: 品番 '{product_number}'",
                        level='warning'
                    )
                    required_inspectors = MAX_INSPECTORS_PER_LOT
                ...
                if force_fixed_assignment and fixed_primary_name:
                    # 長時間ロットは固定検査員の分散を許容し、1名固定を強制しない
                    if required_inspectors > 1:
                        force_fixed_assignment = False
                        self.log_message(
                            f"固定検査員の強制単独割当を解除: 品番 '{product_number}'（必要人数: {required_inspectors}人）",
                            level='info'
                        )
                    else:
                        required_inspectors = 1
                ...
                # 特例: 一ロットで検査員が最大人数を超える場合は上限に制限
                
                process_name_context = self._get_tuple_value(row, result_cols_after_sort, '現在工程名')
                current_date_for_assign = pd.Timestamp.now().date()
                lot_date_for_assign = self._resolve_lot_date(shipping_date, current_date_for_assign)
                ignore_product_limit_for_lot = (
                    force_full_pool_for_product or self._should_force_assign_same_day(shipping_date)
                )
                if fixed_inspector_names and inspection_time > self.product_limit_hard_threshold:
                ...
                if force_fixed_assignment and fixed_primary_name:
                    fixed_name = str(fixed_primary_name).strip()
                    if fixed_name and not any(str(insp.get('氏名', '')).strip() == fixed_name for insp in assigned_inspectors):
                        if not self.is_inspector_on_vacation(fixed_name):
                            ...
                            if not inspector_info.empty:
                                ...
                                fixed_candidate = {
                                    ...
                                    'コード': inspector_row.get('#ID', ''),
                                    '割当時間': float(inspection_time),
                                    'is_fixed_inspector': True,
                                }
                            else:
                                fixed_candidate = {
                                    '氏名': fixed_name,
                            ...
                            current_date = current_date_for_assign
                            fixed_code = str(fixed_candidate.get('コード', '')).strip()
                            max_hours = self.get_inspector_max_hours(fixed_code, inspector_master_df)
                            allowed_max_hours = self._apply_work_hours_overrun(max_hours)
                            current_daily_hours = self.inspector_daily_assignments.get(fixed_code, {}).get(lot_date_for_assign, 0.0)
                            assignable_time = max(0.0, allowed_max_hours - WORK_HOURS_BUFFER - current_daily_hours)
                            assigned_time = min(float(inspection_time), assignable_time)
                            if assigned_time <= 0:
                                self.log_message(
                                    f"警告: 固定検査員 '{fixed_name}' は勤務時間上限により割当できません（上限: {allowed_max_hours:.1f}h）",
                                    level='warning'
                                )
                            else:
                                fixed_candidate['割当時間'] = assigned_time
                                assigned_inspectors = [fixed_candidate]
                                remaining_time = max(0.0, float(inspection_time) - assigned_time)
                                assigned_time_sum = assigned_time
                                self.log_message(
                                    f"固定検査員 '{fixed_name}' を上限内で割り当てました（登録済み品番の特別処置、割当: {assigned_time:.1f}h）"
                                )
                ...
                # Fixed-inspector hard guard for registered products
                if fixed_inspector_names:
                    fixed_names_set = {
                        self._normalize_person_name(n)
                        for n in fixed_inspector_names
                    ...
                    if assigned_inspectors:
                        has_fixed = any(
                            self._normalize_person_name(insp.get('氏名', '')) in fixed_names_set
                            for insp in assigned_inspectors
                        )
                    # 長時間ロットは固定検査員の単独強制を行わない
                    if (not has_fixed) and fixed_names_set and required_inspectors == 1:
                        preferred_name = str(fixed_primary_name).strip() if fixed_primary_name else None
                        if not preferred_name:
                            preferred_name = next(
                                (n for n in fixed_inspector_names if self._normalize_person_name(n) in fixed_names_set),
                                ""
                            )
                        if preferred_name and not self.is_inspector_on_vacation(preferred_name):
                            inspector_info = self._get_inspector_by_name(preferred_name, inspector_master_df)
                            if not inspector_info.empty:
                                inspector_row = inspector_info.iloc[0]
                                fixed_candidate = {
                                    '氏名': preferred_name,
                            ...
                            if assigned_time <= 0:
                                self.log_message(
                                    f"警告: 固定検査員 '{preferred_name}' は勤務時間上限により割当できません（上限: {allowed_max_hours:.1f}h）",
                                    level='warning'
                                )
                            else:
                                fixed_candidate['割当時間'] = assigned_time
                                ...
                                assigned_time_sum = assigned_time
                                self.log_message(
                                    f"固定検査員 '{preferred_name}' を上限内で割り当てました（最終ガード、割当: {assigned_time:.1f}h）"
                                )
                ...
                if is_same_day_cleaning and inspection_time > self.required_inspectors_threshold and len(assigned_inspectors) < required_inspectors:
                    ...
                    if len(reassigned_inspectors) < required_inspectors:
                        ...
                        for candidate in selected_candidates:
                            ...
                            code = candidate['コード']
                            daily_hours = max(self.inspector_daily_assignments.get(code, {}).values(), default=0.0)
                ...
                if len(assigned_inspectors) == 0:
                    ...
                    if not available_inspectors:
                        ...
                        shipping_date_for_filter = row[result_cols_after_sort.get('出荷予定日', -1)] if '出荷予定日' in result_cols_after_sort else None
                        lot_date_for_filter = self._resolve_lot_date(shipping_date_for_filter, pd.Timestamp.now().date())
                ...
                # 結果を設定（表示上限を超える場合は切り詰める）
                if len(assigned_inspectors) > MAX_INSPECTORS_PER_LOT:
                    self.log_message(
                        f"割当人数 {len(assigned_inspectors)}人が表示上限 {MAX_INSPECTORS_PER_LOT}人を超えるため、"
                        f"{MAX_INSPECTORS_PER_LOT}人に切り詰めます: 品番 '{product_number}'",
                        level='warning'
                    )
                    assigned_inspectors = assigned_inspectors[:MAX_INSPECTORS_PER_LOT]
                ...
                shipping_date_raw = result_df.at[index, '出荷予定日'] if '出荷予定日' in result_df.columns else None
                lot_date_for_rebuild = self._resolve_lot_date(shipping_date_raw, current_date)
                ...
                for i, inspector in enumerate(assigned_inspectors):
                    if i < MAX_INSPECTORS_PER_LOT:  # 最大人数まで
                        ...
                        if lot_date_for_rebuild not in self.inspector_daily_assignments[code]:
                            self.inspector_daily_assignments[code][lot_date_for_rebuild] = 0.0
                        self.inspector_daily_assignments[code][lot_date_for_rebuild] += assigned_time
                            ...
                            if code in already_assigned_to_this_product and len(already_assigned_to_this_product) > 1:
                                ...
                                other_lots_with_same_inspector = [
                                    ...
                                    and any(
                                        ...
                                        for j in range(1, MAX_INSPECTORS_PER_LOT + 1)
                                    )
                                ]
    ...
    def get_available_inspectors(
        ...
        try:
            ...
            new_team_cache = getattr(self, "_new_team_inspectors_cache", None)
            if new_team_cache is None:
                new_team_cache = self.get_new_product_team_inspectors(inspector_master_df)
                self._new_team_inspectors_cache = new_team_cache
            new_team_inspectors = new_team_cache
            ...
            skill_rows = None
            skill_cache = getattr(self, "_skill_rows_by_product_cache", None)
            if isinstance(skill_cache, dict):
                skill_rows = skill_cache.get(product_number)
            if skill_rows is None:
                skill_rows = skill_master_df[skill_master_df.iloc[:, 0] == product_number]
                if isinstance(skill_cache, dict):
                    skill_cache[product_number] = skill_rows
            ...
            if not filtered_skill_rows:
                ...
                if allow_new_team_fallback and shipping_date is not None:
                    ...
                    if pd.notna(shipping_date):
                        shipping_date_date = shipping_date.date()
                        current_date = pd.Timestamp.now().date()
                        lot_date_for_relax = self._resolve_lot_date(shipping_date, current_date)
                        two_weeks_later = current_date + timedelta(days=14)
                        if shipping_date_date <= two_weeks_later:
                            warning_key = (
                                "new_team_fallback",
            ...
            if available_inspectors:
                ...
                for insp in available_inspectors:
                    is_fixed = (
                        self._normalize_person_name(insp['氏名']) in fixed_name_norms
                        if fixed_inspector_names
                        else False
                    )
    ...
    def assign_inspectors_asymmetric(
        ...
        try:
            ...
            current_date = pd.Timestamp.now().date()
            target_date = lot_date or current_date
            ...
            for inspector in available_inspectors:
                ...
                # 残り勤務時間を計算
                daily_hours = self.inspector_daily_assignments.get(code, {}).get(target_date, 0.0)
                max_hours = self.get_inspector_max_hours(code, inspector_master_df)
                allowed_max_hours = self._apply_work_hours_overrun(max_hours)
            ...
            for candidate in candidates_with_capacity:
                ...
                # 実際の割り当て時点での勤務時間を再計算
                current_daily_hours = self.inspector_daily_assignments.get(code, {}).get(target_date, 0.0)
    ...
    def select_inspectors(
        ...
        try:
            # 特例: 一ロットで検査員が最大人数を超える場合は上限に制限
            if required_count > MAX_INSPECTORS_PER_LOT:
                required_count = MAX_INSPECTORS_PER_LOT
            ...
            current_time = pd.Timestamp.now()
            current_date = current_time.date()
            lot_date_for_rebuild = lot_date or current_date
            target_date = lot_date_for_rebuild
            target_process_name = process_name_context or ''
            ...
            for inspector in available_inspectors:
                ...
                if lot_date_for_rebuild not in self.inspector_daily_assignments[inspector_code]:
                    self.inspector_daily_assignments[inspector_code][lot_date_for_rebuild] = 0.0
            ...
            available_inspectors = self.filter_available_inspectors(
                ...
                lot_date=lot_date_for_rebuild,
            )
            filtered_count = len(available_inspectors)
            if input_count > 0 and filtered_count == 0:
                # 緩和候補が全て除外された場合、詳細ログを出力
                mode_str = "緩和モード（10%超過許容）" if relax_work_hours else "通常モード（10%超過許容）"
                warning_key = (
                    "reprocess_all_excluded",
                    str(product_number).strip(),
                    mode_str,
                    ignore_product_limit,
                    input_count,
                )
                if warning_key not in self.logged_warnings:
                    self.logged_warnings.add(warning_key)
                    self.log_message(
                        f"警告: 未割当ロット再処理: 品番 {product_number} の候補 {input_count}名がfilter_available_inspectorsで全て除外されました（{mode_str}, ignore_product_limit={ignore_product_limit}）",
                        level='warning',
                    )
            ...
            if not filtered_by_product:
                if ignore_product_limit:
                    ...
                    warning_key = (
                        "reprocess_product_limit_excluded",
                        str(product_number).strip(),
                        bool(is_new_product),
                    )
                    if warning_key not in self.logged_warnings:
                        ...
                        if is_new_product:
                            self.log_message(
                                f"警告: 新規品 {product_number} の4時間上限または勤務時間上限により全員が除外。ルール違反を避けるため、このロットは未割当とします"
                            )
                        else:
                            self.log_message(
                                f"警告: 品番 {product_number} の4時間上限により全員が除外。ルール違反を避けるため、このロットは未割当とします"
                            )
                    return []
            ...
            selected_inspectors = self.select_inspectors_with_skill_combination(
                filtered_by_product, required_count, divided_time, current_time, target_date, inspector_master_df, product_number,
                process_name_context=target_process_name,
    ...
    def select_inspectors_with_skill_combination(
        ...
        try:
            ...
            fixed_name_norms = {
                ...
                for n in fixed_inspector_names
                if self._normalize_person_name(n)
            } if fixed_inspector_names else set()
            ...
            for inspector in available_inspectors:
                # 固定検査員フラグを設定
                inspector_name = inspector.get('氏名', '')
                inspector['__is_fixed'] = (
                    self._normalize_person_name(inspector_name) in fixed_name_norms
                    if fixed_name_norms
                    else False
                )
            ...
            if required_count == 1:
                ...
                all_inspectors_with_priority.sort(key=self._priority_sort_key)
            # 特例: 一ロットで検査員が最大人数を超える場合は上限に制限
                max_count = min(MAX_INSPECTORS_PER_LOT, required_count)
                # 【改善】10%超過をチェックしながら選択
                for priority, insp in all_inspectors_with_priority:
            ...
            for insp in selected_inspectors:
                ...
                if exceeded:
                    mode_text = "緩和モードでも10%超過" if relax_work_hours else "10%超過"
                    self.log_message(
    ...
    def filter_available_inspectors(
        ...
        try:
            ...
            for inspector in available_inspectors:
                ...
                # 対象日の累積勤務時間を取得
                daily_hours = self.inspector_daily_assignments.get(inspector_code, {}).get(target_date, 0.0)
                ...
                # 緩和モードの場合も10%超過まで許容（当日洗浄上がり品の制約緩和時など）
                if relax_work_hours:
                    # 緩和モード: 10%超過まで許容（SAME_DAY_WORK_HOURS_OVERRUN_RATEを使用）
                    ...
                    if daily_hours + additional_hours > relaxed_allowed_max_hours - WORK_HOURS_BUFFER:
                        ...
                        self.log_message(
                            f"検査員 '{inspector['氏名']}' は緩和モードでも10%超過を超えるため除外 "
                ...
                if not ignore_product_limit:
                    ...
                    inspector_entry['__current_product_hours'] = product_hours
                    inspector_entry['__projected_product_hours'] = projected_hours
                else:
            ...
            if input_count > 0 and filtered_count == 0:
                ...
                warning_key = (
                    ...
                    mode_str,
                    ignore_product_limit,
                    len(excluded_by_vacation),
                    len(excluded_by_work_hours),
                    len(excluded_by_product_limit),
                )
                if warning_key not in self.logged_warnings:
                    self.logged_warnings.add(warning_key)
                    self.log_message(
                        f"filter_available_inspectors: 品番 {product_number} の入力 {input_count}名が全て除外されました（休暇: {len(excluded_by_vacation)}名、勤務時間: {len(excluded_by_work_hours)}名、4時間上限: {len(excluded_by_product_limit)}名、{mode_str}, ignore_product_limit={ignore_product_limit}）",
                        level='warning',
                    )

            # 品番上限に近い検査員を後回しにする（早期分散）
            if filtered_inspectors:
                filtered_inspectors.sort(
                    key=lambda insp: (
                        insp.get('over_product_limit', False),
                        insp.get('__projected_product_hours', 0.0),
                        insp.get('__current_product_hours', 0.0),
                        str(insp.get('氏名', '')),
                    )
                )
    ...
    def optimize_assignments(
        ...
        try:
            ...
            for row_idx, row in enumerate(result_df.itertuples(index=False)):
                ...
                shipping_date_val = row[ship_date_col_idx] if ship_date_col_idx != -1 else None
                lot_date_for_rebuild = self._resolve_lot_date(shipping_date_val, current_date)
            ...
            inspector_col_idxs = [sorted_cols.get(f'検査員{i}', -1) for i in range(1, MAX_INSPECTORS_PER_LOT + 1)]
            two_weeks_later = current_date + timedelta(days=14)
            ...
            for row_idx, row in enumerate(result_df.itertuples(index=False)):
                ...
                if divided_time == -1:
                    divided_time = 0.0
                shipping_date_val = row[final_cols.get('出荷予定日', -1)] if '出荷予定日' in final_cols else None
                lot_date_for_check = self._resolve_lot_date(shipping_date_val, current_date)
                    ...
                    if inspector_col_idx != -1:
                        ...
                        if pd.notna(inspector_name_raw) and str(inspector_name_raw).strip() != '':
                            ...
                            if daily_hours > allowed_max_hours - WORK_HOURS_BUFFER:
                                ...
                                mode_str = "緩和モード（10%超過許容）" if is_same_day_cleaning else "通常モード（10%超過許容）"
            ...
            # 特例: 一ロットで検査員が最大人数を超える場合は上限に制限
                            if inspection_time <= 0:
                                ...
                                if inspection_time <= self.required_inspectors_threshold:
                                    ...
                                    # 最大人数を超える場合は上限に制限（特例）
                                    required_inspectors = min(MAX_INSPECTORS_PER_LOT, calculated_inspectors)
                            divided_time = inspection_time / required_inspectors
                            
                            # 新製品チームから検査員を選択
                            lot_date_for_assign = self._resolve_lot_date(shipping_date, current_date)
                ...
                ship_date_col_recalc = recalc_cols.get('出荷予定日', -1)
                for row_idx, row in enumerate(result_df.itertuples(index=False)):
                    index = result_df.index[row_idx]
                    ...
                    if divided_time == -1:
                        divided_time = 0.0
                    shipping_date_val = row[ship_date_col_recalc] if ship_date_col_recalc != -1 else None
                    lot_date_for_recalc = self._resolve_lot_date(shipping_date_val, current_date)
                        ...
                        if inspector_col_idx != -1:
                            ...
                            if pd.notna(inspector_name_raw) and str(inspector_name_raw).strip() != '':
                                ...
                                if not inspector_info.empty:
                                    ...
                                    if inspector_code not in self.inspector_daily_assignments:
                                        self.inspector_daily_assignments[inspector_code] = {}
                                    if lot_date_for_recalc not in self.inspector_daily_assignments[inspector_code]:
                                        self.inspector_daily_assignments[inspector_code][lot_date_for_recalc] = 0.0
                                    if inspector_code not in self.inspector_work_hours:
                                        self.inspector_work_hours[inspector_code] = 0.0
                                    if inspector_code not in self.inspector_product_hours:
                                        self.inspector_product_hours[inspector_code] = {}
                                    ...
                                    self.inspector_daily_assignments[inspector_code][lot_date_for_recalc] += divided_time
                                    self.inspector_work_hours[inspector_code] += divided_time
                                    self.inspector_product_hours[inspector_code][product_number] += divided_time
            else:
            ...
            # 再割当て後の履歴が重複集計されるのを防ぐため、ここで必ず再構築する。
            try:
                self._rebuild_assignment_histories(result_df, inspector_master_df)
            except Exception as e:
                self.log_message(f"偏り是正前の履歴再構築でエラーが発生しました: {e}", level='warning')
            ...
            if active_inspectors:
                ...
                if imbalance > imbalance_threshold and len(active_inspectors) > 1:
                    ...
                    if over_loaded and under_loaded:
                        ...
                        def add_business_days_for_bias(start: date, days: int) -> date:
                            """Return date after N business days (Mon-Fri)."""
                            result = start
                            added = 0
                            while added < days:
                        ...
                        bias_protect_until = add_business_days_for_bias(current_date, 3)
                        bias_protect_until_ts = pd.Timestamp(bias_protect_until)
                        ...
                        for pass_num in range(max_passes):
                            ...
                            if pass_num > 0:  # 最初のパス以外は再計算
                                ...
                                self.inspector_work_hours = {}
                                self.inspector_product_hours = {}
                                ship_date_col_bias = result_cols_bias.get('出荷予定日', -1)
                                ...
                                for row_idx, row in enumerate(result_df_sorted.itertuples(index=False)):
                                    ...
                                    # 各検査員の割り当てを確認
                                    shipping_date_val = row[ship_date_col_bias] if ship_date_col_bias != -1 else None
                                    lot_date_for_bias = self._resolve_lot_date(shipping_date_val, current_date)
                                    for i in range(1, MAX_INSPECTORS_PER_LOT + 1):
                                        inspector_col = f'検査員{i}'
                                        inspector_col_idx = result_cols_bias.get(inspector_col, -1)
                                        if inspector_col_idx != -1:
                                            ...
                                            if pd.notna(inspector_name_raw) and str(inspector_name_raw).strip() != '':
                                                ...
                                                if not inspector_info.empty:
                                                    ...
                                                    if inspector_code not in self.inspector_daily_assignments:
                                                        self.inspector_daily_assignments[inspector_code] = {}
                                                    if lot_date_for_bias not in self.inspector_daily_assignments[inspector_code]:
                                                        self.inspector_daily_assignments[inspector_code][lot_date_for_bias] = 0.0
                                                    if inspector_code not in self.inspector_work_hours:
                                                        self.inspector_work_hours[inspector_code] = 0.0
                                                    if inspector_code not in self.inspector_product_hours:
                                                        self.inspector_product_hours[inspector_code] = {}
                                                    ...
                                                    # 履歴を累積
                                                    self.inspector_daily_assignments[inspector_code][lot_date_for_bias] += divided_time
                                                    self.inspector_work_hours[inspector_code] += divided_time
                                                    self.inspector_product_hours[inspector_code][product_number] += divided_time
                        ...
                        for overloaded_code, overloaded_hours in over_loaded_pass:
                            ...
                            shipping_date_col_idx = result_df_sorted.columns.get_loc('出荷予定日') if '出荷予定日' in result_df_sorted.columns else -1
                            inspector_col_indices = [result_df_sorted.columns.get_loc(f'検査員{i}') for i in range(1, MAX_INSPECTORS_PER_LOT + 1)]
                            ...
                            for row_tuple in result_df_sorted.itertuples(index=True):
                                ...
                                shipping_date_raw = None
                                if shipping_date_col_idx >= 0 and shipping_date_col_idx < len(row_tuple) - 1:
                                    shipping_date_raw = row_tuple[shipping_date_col_idx + 1]
                                    ...
                                    if pd.notna(inspector_value) and str(inspector_value).strip() != '':
                                        ...
                                        if not inspector_info.empty:
                                            ...
                                            if lot_inspector_code == overloaded_code:
                                                # Protect earlier lots to keep shipping-date priority.
                                                normalized_ship_date = self._normalize_shipping_date(shipping_date_raw)
                                                if normalized_ship_date <= bias_protect_until_ts:
                                                    continue
                                                # Seriesとして扱うために元の行を取得
                            ...
                            for lot_index, product_number, divided_time, inspector_col_num, row in assigned_lots:
                                ...
                                if fixed_inspector_names:
                                    ...
                                    if pd.notna(current_inspector_value) and str(current_inspector_value).strip() != '':
                                        ...
                                        if self._normalize_person_name(current_inspector_name) in fixed_name_norms:
                                ...
                                for insp in available_inspectors:
                                    ...
                                    if fixed_inspector_names:
                                        ...
                                        if self._normalize_person_name(candidate_name) in fixed_name_norms:
                                            # 固定検査員は置き換え先候補から除外
                                            continue
            ...
            inspector_col_indices = [result_df.columns.get_loc(f'検査員{i}') for i in range(1, MAX_INSPECTORS_PER_LOT + 1)]
            ...
            if phase2_5_violations:
                ...
                for product_number, product_violations in violations_by_product.items():
                    if len(product_violations) > 1:
                        violation_types = {
                            str(v[0][3]) if len(v[0]) > 3 else ''
                            for v in product_violations
                        }
                        if violation_types and violation_types.issubset({"勤務時間超過"}):
                            self.log_message(
                                f"🔵 品番 {product_number} の違反は勤務時間超過のみのため、まとめてクリアはせず個別是正を試みます"
                            )
                            continue
            ...
            if same_day_cleaning_unassigned:
                ...
                for unassigned_index in same_day_cleaning_unassigned:
                    ...
                    if reassigned_inspectors:
                        ...
                        # 検査員を割り当て
                        for i, inspector in enumerate(reassigned_inspectors[:MAX_INSPECTORS_PER_LOT], 1):  # 最大人数まで
                            inspector_name = inspector['氏名']
                            inspector_code = inspector['コード']
            ...
            if unassigned_indices:
                ...
                for row_tuple in unassigned_df.itertuples(index=True):
                    ...
                    # まず通常の条件で試行
                    lot_date_for_assign = self._resolve_lot_date(shipping_date, pd.Timestamp.now().date())
                    assigned_inspectors = self.select_inspectors(
                        available_inspectors,
                        required_inspectors,
                        divided_time,
            ...
            # 分割検査時間の再計算後に履歴を再構築（最終検証の精度を担保）
            self.inspector_daily_assignments = {}
            self.inspector_work_hours = {}
            self.inspector_product_hours = {}

            prod_num_col_idx_rebuild = result_df.columns.get_loc('品番')
            div_time_col_idx_rebuild = result_df.columns.get_loc('分割検査時間') if '分割検査時間' in result_df.columns else -1
            shipping_date_col_idx_rebuild = result_df.columns.get_loc('出荷予定日') if '出荷予定日' in result_df.columns else -1
            inspector_col_indices_rebuild = {}
            for i in range(1, MAX_INSPECTORS_PER_LOT + 1):
                col_name = f'検査員{i}'
                if col_name in result_df.columns:
                    inspector_col_indices_rebuild[i] = result_df.columns.get_loc(col_name)

            for row_tuple in result_df.itertuples(index=False):
                product_number = row_tuple[prod_num_col_idx_rebuild]
                divided_time = row_tuple[div_time_col_idx_rebuild] if div_time_col_idx_rebuild >= 0 and div_time_col_idx_rebuild < len(row_tuple) else 0.0

                # 出荷予定日から日付を取得（当日洗浄上がり品の場合は今日の日付を使用）
                lot_date_for_rebuild = current_date  # デフォルトは今日の日付
                if shipping_date_col_idx_rebuild >= 0 and shipping_date_col_idx_rebuild < len(row_tuple):
                    shipping_date_raw = row_tuple[shipping_date_col_idx_rebuild]
                    lot_date_for_rebuild = self._resolve_lot_date(shipping_date_raw, current_date)

                for i in range(1, MAX_INSPECTORS_PER_LOT + 1):
                    if i not in inspector_col_indices_rebuild:
                        continue
                    inspector_col_idx = inspector_col_indices_rebuild[i]
                    inspector_value = row_tuple[inspector_col_idx] if inspector_col_idx < len(row_tuple) else None
                    ...
                    if pd.notna(inspector_value) and str(inspector_value).strip() != '':
                        ...
                        if not inspector_info.empty:
                            inspector_code = inspector_info.iloc[0]['#ID']
                            self.inspector_daily_assignments.setdefault(inspector_code, {}).setdefault(lot_date_for_rebuild, 0.0)
                            self.inspector_work_hours.setdefault(inspector_code, 0.0)
                            self.inspector_product_hours.setdefault(inspector_code, {}).setdefault(product_number, 0.0)
                ...
                for violation in same_day_cleaning_violations:
                    ...
                    if violating_inspector_col:
                        ...
                        if fixed_inspector_names:
                            ...
                            if self._normalize_person_name(violation_name) in fixed_name_norms:
                                self.log_message(
            ...
            if phase3_5_violations:
                ...
                for product_number, product_violations in violations_by_product.items():
                    if len(product_violations) > 1:
                        ...
                        for violation, row in product_violations:
                            ...
                            if self._is_same_day_cleaning_label(shipping_date):
                                self.log_message(
                                    f"⚠️ 当日洗浄上がり品は最優先のため、フェーズ3.5の未割当対象から除外します: "
                            ...
                            if False and (is_within_three_business_days or is_within_two_weeks):
                                inspector_code = violation[1]
                                violation_type = violation[3]
                            ...
                            if False and is_new_product and is_within_two_weeks:
                                inspector_code = violation[1]
                                violation_type = violation[3]
                ...
                for violation, shipping_date in violations_with_date:
                    ...
                    inspection_time = row.get('検査時間', 0)
                    shipping_date_raw = row.get('出荷予定日', pd.Timestamp.max)
                    if self._is_same_day_cleaning_label(shipping_date_raw):
                        self.log_message(
                            f"⚠️ 当日洗浄上がり品は最優先のため、フェーズ3.5の未割当対象から除外します: "
            ...
            # 列インデックスを事前に取得（itertuples()で高速化）
            # Enforce fixed inspectors for registered products before final recalculation
            try:
                inspector_prefix = "\u691c\u67fb\u54e1"  # ???
                col_product = "\u54c1\u756a"  # ??
                col_process = "\u73fe\u5728\u5de5\u7a0b\u540d"  # ?????
                col_lot_id = "\u751f\u7523\u30ed\u30c3\u30c8ID"  # ?????ID
                inspector_cols = [
                    f"{inspector_prefix}{i}"
                    for i in range(1, MAX_INSPECTORS_PER_LOT + 1)
                    if f"{inspector_prefix}{i}" in result_df.columns
                ]
                for col in inspector_cols:
                    try:
                        if not (pd.api.types.is_object_dtype(result_df[col]) or pd.api.types.is_string_dtype(result_df[col])):
                ...
                if inspector_cols:
                    ...
                    for idx, row in result_df.iterrows():
                        product_number = row.get(col_product, "")
                        if not product_number or pd.isna(product_number):
                            continue
                        process_name = row.get(col_process, "")
                        fixed_names = self._collect_fixed_inspector_names(str(product_number).strip(), str(process_name).strip())
                        if not fixed_names:
                            continue
                        # Filter out full-day vacation inspectors
                        fixed_names = [n for n in fixed_names if not self.is_inspector_on_vacation(n)]
                        if not fixed_names:
                            continue
                        key = str(product_number).strip()
                        entry = product_rows.setdefault(key, {"fixed_names": fixed_names, "rows": []})
                        entry["rows"].append(idx)

                    for product_key, entry in product_rows.items():
                        fixed_names = entry["fixed_names"]
                        fixed_norm_map = {
                            self._normalize_person_name(n): str(n).strip()
                            for n in fixed_names
                            if self._normalize_person_name(n)
                        }
                        fixed_set = set(fixed_norm_map.keys())
                        if not fixed_set:
                            continue
                        row_ids = entry["rows"]
                        row_infos = []
                        for idx in row_ids:
                            ...
                            for col in inspector_cols:
                                val = row.get(col, "")
                                if pd.notna(val) and str(val).strip() != "":
                                    assigned_name = str(val).strip()
                                    break
                            if "(" in assigned_name:
                                assigned_name = assigned_name.split("(")[0].strip()
                            assigned_norm = self._normalize_person_name(assigned_name)
                            row_infos.append((idx, assigned_name, assigned_norm))

                        # Ensure each row has a fixed inspector assigned
                        for idx, assigned_name, assigned_norm in row_infos:
                            if assigned_norm and assigned_norm in fixed_set:
                                continue
                            # Round-robin among fixed inspectors per product to avoid bias
                            count = fixed_rotation.get(product_key, 0)
                            preferred_name = str(fixed_names[count % len(fixed_names)]).strip()
                            fixed_rotation[product_key] = count + 1
                            if not preferred_name:
                                continue
                            result_df.at[idx, inspector_cols[0]] = preferred_name
                            self.update_team_info(result_df, idx, inspector_master_df, show_skill_values)
                            self.log_message(
                                f"Fixed inspector final adjust: product={product_key}, name={preferred_name}"
                            )

                        # If lot count allows, ensure all fixed inspectors appear at least once
                        row_infos = []
                        for idx in row_ids:
                            row = result_df.loc[idx]
                            assigned_name = ""
                        ...
                        if len(row_infos) >= len(fixed_set):
                            counts: Dict[str, int] = {}
                            for _, _, assigned_norm in row_infos:
                                if assigned_norm in fixed_set:
                                    counts[assigned_norm] = counts.get(assigned_norm, 0) + 1
                            missing_norms = [n for n in fixed_set if n not in counts]
                            if missing_norms:
                                used_rows = set()
                                for missing_norm in missing_norms:
                                    preferred_name = fixed_norm_map.get(missing_norm, "")
                                    if not preferred_name:
                                        continue
                                    # Prefer rows with non-fixed or duplicated fixed assignments
                                    candidates = []
                                    for idx, _, assigned_norm in row_infos:
                                        if idx in used_rows:
                                            continue
                                        if assigned_norm not in fixed_set:
                                            candidates.append((0, idx))
                                        else:
                                            candidates.append((1, idx, counts.get(assigned_norm, 0)))
                                    if not candidates:
                                        continue
                                    candidates.sort(key=lambda c: (c[0], -c[2] if len(c) > 2 else 0))
                                    target_idx = candidates[0][1]
                                    result_df.at[target_idx, inspector_cols[0]] = preferred_name
                                    self.update_team_info(result_df, target_idx, inspector_master_df, show_skill_values)
                                    used_rows.add(target_idx)
                                    counts[missing_norm] = counts.get(missing_norm, 0) + 1
                                    self.log_message(
                                        f"Fixed inspector coverage adjust: product={product_key}, name={preferred_name}"
                                    )
            except Exception as e:
                self.log_message(f"Fixed inspector final adjust error: {e}")
            ...
            # 固定検査員の最終調整後、勤務時間上限を厳密に満たすように再調整
            try:
                self._rebuild_assignment_histories(result_df, inspector_master_df)
                inspector_cols = [
                    f'検査員{i}'
                    for i in range(1, MAX_INSPECTORS_PER_LOT + 1)
                    if f'検査員{i}' in result_df.columns
                ]
                while True:
                    over_limit = []
                    for inspector_code, daily_map in self.inspector_daily_assignments.items():
                        max_hours = self.get_inspector_max_hours(inspector_code, inspector_master_df)
                        allowed_max_hours = self._apply_work_hours_overrun(max_hours)
                        for lot_date, daily_hours in daily_map.items():
                            if daily_hours > allowed_max_hours:
                                over_limit.append((inspector_code, lot_date, daily_hours, allowed_max_hours))

                    if not over_limit:
                        break

                    over_limit.sort(key=lambda x: (x[2] - x[3], x[2]), reverse=True)
                    inspector_code, target_lot_date, daily_hours, allowed_max_hours = over_limit[0]
                    inspector_name = str(inspector_code).strip()
                    inspector_info = pd.DataFrame()
                    if inspector_master_df is not None and not inspector_master_df.empty:
                        if '#ID' in inspector_master_df.columns:
                            inspector_info = inspector_master_df[inspector_master_df['#ID'] == inspector_code]
                        elif '#コード' in inspector_master_df.columns:
                            inspector_info = inspector_master_df[inspector_master_df['#コード'] == inspector_code]
                        if not inspector_info.empty and '#氏名' in inspector_master_df.columns:
                            inspector_name = str(inspector_info.iloc[0]['#氏名']).strip()
                    inspector_norm = self._normalize_person_name(inspector_name)
                    if not inspector_norm:
                        self.log_message(
                            f"勤務時間超過の是正不可: 検査員コード {inspector_code}",
                            level='warning',
                        )
                        break
                    ...
                    for idx, row in result_df.iterrows():
                        ...
                        for col in inspector_cols:
                            ...
                            if self._normalize_person_name(assigned_clean) != inspector_norm:
                                continue
                            if self._is_fixed_inspector_for_lot(product_number, process_name, inspector_name):
                                break
                            shipping_date = row.get('出荷予定日', pd.Timestamp.max)
                            ...
                            if self._is_same_day_cleaning_label(shipping_date):
                                break
                            candidates.append((self._normalize_shipping_date(shipping_date), idx))
                            break

                    if not candidates:
                        self.log_message(
                            f"勤務時間超過の是正不可: {inspector_name}({inspector_code}) の解除可能ロットがありません",
                            level='warning',
                        )
                        break
                    ...
                    self.log_message(
                        f"勤務時間超過の是正: {inspector_name}({inspector_code}) のロットを未割当にしました (index {target_idx})",
                        level='warning',
                    )
                    self._rebuild_assignment_histories(result_df, inspector_master_df)
            except Exception as e:
                self.log_message(f"勤務時間超過の最終是正中にエラーが発生しました: {e}", level='warning')

            # 最終是正後に偏り是正を1回だけ実行（超過を出さず、優先度を維持）
            try:
                result_df = self._run_post_final_bias_correction(
                    result_df,
                    inspector_master_df,
                    skill_master_df,
                ...
                self.log_message(f"最終是正後の偏り是正中にエラーが発生しました: {e}", level='warning')

            # 最終是正後に余裕検査員へ追加割当（10%超過なし、偏り悪化なし）
            try:
                ...
                self.log_message(f"最終是正後の追加割当中にエラーが発生しました: {e}", level='warning')
            ...
            for row_tuple in result_df.itertuples(index=True):
                ...
                try:
                    inspector_count = float(inspector_count) if pd.notna(inspector_count) else 0
                except Exception:
                    inspector_count = 0
    ...
    def fix_single_violation(
        ...
        try:
            ...
            shipping_date = None
            shipping_date_raw = row.get('出荷予定日', None)
            lot_date_for_violation = self._resolve_lot_date(shipping_date_raw, current_date)
            is_within_two_weeks = False
            ...
            if len(current_inspectors) > 1:
                ...
                if removed_inspector:
                    ...
                    for level in relaxation_levels:
                        ...
                        for insp in available_inspectors:
                            if insp['コード'] not in current_codes:
                                ...
                                max_hours = inspector_max_hours.get(code, 8.0)
                                daily_hours = self.inspector_daily_assignments.get(code, {}).get(lot_date_for_violation, 0.0)
                    ...
                    if replacement_candidates:
                        ...
                        old_daily = self.inspector_daily_assignments.get(old_code, {}).get(lot_date_for_violation, 0.0)
                        old_total = self.inspector_work_hours.get(old_code, 0.0)
                        self.inspector_daily_assignments[old_code][lot_date_for_violation] = max(0.0, old_daily - divided_time)
                        ...
                        if new_code not in self.inspector_daily_assignments:
                            self.inspector_daily_assignments[new_code] = {}
                        if lot_date_for_violation not in self.inspector_daily_assignments[new_code]:
                            self.inspector_daily_assignments[new_code][lot_date_for_violation] = 0.0
                        
                        self.inspector_daily_assignments[new_code][lot_date_for_violation] += divided_time
                        if new_code not in self.inspector_work_hours:
                            self.inspector_work_hours[new_code] = 0.0
                        self.inspector_work_hours[new_code] += divided_time
                ...
                if inspection_time < self.required_inspectors_threshold:
                    ...
                    if replacement_candidates:
                        ...
                        old_daily = self.inspector_daily_assignments.get(old_code, {}).get(lot_date_for_violation, 0.0)
                        old_total = self.inspector_work_hours.get(old_code, 0.0)
                        self.inspector_daily_assignments[old_code][lot_date_for_violation] = max(0.0, old_daily - inspection_time)
                        ...
                        self.inspector_daily_assignments[new_code][lot_date_for_violation] += inspection_time
                        if new_code not in self.inspector_work_hours:
                            self.inspector_work_hours[new_code] = 0.0
                    ...
                    for insp in available_inspectors:
                        if insp['コード'] not in current_codes:
                            ...
                            max_hours = inspector_max_hours.get(code, 8.0)
                            # 勤務時間チェック
                            if not self.check_work_hours_capacity(code, inspection_time, max_hours, lot_date_for_violation):
                                daily_hours = self.inspector_daily_assignments.get(code, {}).get(lot_date_for_violation, 0.0)
                    ...
                    # 検査員を最大人数まで増員を試みる（3時間の条件は無視）
                    # 現在の検査員が上限未満の場合のみ増員を試みる
                    if len(current_inspectors) < MAX_INSPECTORS_PER_LOT:
                        process_number = row.get('現在工程番号', '')
                        lot_process_name = str(row.get('現在工程名', '') or '').strip()
                        # スキルマスタに登録があるか確認
                        skill_rows = skill_master_df[skill_master_df.iloc[:, 0] == product_number]
                    ...
                    for insp in available_inspectors:
                        if insp['コード'] not in current_codes:
                            ...
                            max_hours = inspector_max_hours.get(code, 8.0)
                            if not self.check_work_hours_capacity(code, divided_time, max_hours, lot_date_for_violation):
                                daily_hours = self.inspector_daily_assignments.get(code, {}).get(lot_date_for_violation, 0.0)
                    ...
                    if not addition_candidates and len(current_inspectors) < MAX_INSPECTORS_PER_LOT:
                    ...
                    if addition_candidates and len(current_inspectors) < MAX_INSPECTORS_PER_LOT:
                        # 最も総勤務時間が少ない人を追加
    ...
    def clear_assignment(self, result_df: pd.DataFrame, index: int) -> None:
        ...
        try:
            ...
            team_info = result_df.at[index, 'チーム情報']
            log_key = ("unassigned_lot", str(product_number).strip(), int(index), str(team_info).strip())
            if log_key not in self.logged_warnings:
                self.logged_warnings.add(log_key)
                self.log_message(
                    f"ロットを未割当にしました: 品番 {product_number}, インデックス {index}, チーム情報: '{team_info}'"
                )
            
        except Exception as e:
            self.log_message(f"未割当処理中にエラーが発生しました: {str(e)}")

    def _run_post_final_bias_correction(
        self,
        result_df: pd.DataFrame,
        inspector_master_df: pd.DataFrame,
        skill_master_df: pd.DataFrame,
        ...
        """
        最終是正後に偏り是正を1回だけ実行（出荷予定日の優先順位と勤務時間上限を維持）。
        """
        self.log_message("全体最適化フェーズ2.6: 最終是正後の偏り是正を開始")
        ...
        try:
            ...
            self.log_message(f"偏り是正(後処理)の履歴再構築でエラーが発生しました: {e}", level='warning')
            return result_df

        current_date = pd.Timestamp.now().date()
        total_hours = sum(self.inspector_work_hours.values())
        active_codes = [c for c, h in self.inspector_work_hours.items() if h > 0]
        if not active_codes:
            return result_df

        avg_hours = total_hours / len(active_codes)
        over_loaded = [(code, self.inspector_work_hours.get(code, 0.0)) for code in active_codes if self.inspector_work_hours.get(code, 0.0) > avg_hours]
        under_loaded = [(code, self.inspector_work_hours.get(code, 0.0)) for code in active_codes if self.inspector_work_hours.get(code, 0.0) < avg_hours]

        if not over_loaded or not under_loaded:
            return result_df

        over_loaded.sort(key=lambda x: x[1], reverse=True)
        under_loaded_set = {code for code, _ in under_loaded}
        self.log_message(
            f"偏り是正(後処理): 多忙 {len(over_loaded)}人 / 余裕 {len(under_loaded)}人, 平均 {avg_hours:.1f}h",
            debug=True,
        )

        def _get_candidate_code(candidate: Dict[str, Any]) -> str:
            return candidate.get('コード', candidate.get('#ID', candidate.get('コーチID', candidate.get('コーチE', ''))))
        ...
        bias_protect_until_ts = pd.Timestamp(bias_protect_until)
        self.log_message(
            f"偏り是正(後処理): 出荷予定日 {bias_protect_until} までのロットは保護",
            debug=True,
        )
        ...
        for overloaded_code, _ in over_loaded:
            ...
            if inspector_master_df is not None and not inspector_master_df.empty:
                if '#ID' in inspector_master_df.columns:
                    inspector_info = inspector_master_df[inspector_master_df['#ID'] == overloaded_code]
                elif '#コード' in inspector_master_df.columns:
                    inspector_info = inspector_master_df[inspector_master_df['#コード'] == overloaded_code]

            overloaded_name = str(overloaded_code).strip()
            if not inspector_info.empty and '#氏名' in inspector_master_df.columns:
                overloaded_name = str(inspector_info.iloc[0]['#氏名']).strip()
            overloaded_norm = self._normalize_person_name(overloaded_name)
            if not overloaded_norm:
                continue
            ...
            for idx, row in result_df.iterrows():
                ...
                if normalized_ship_date <= bias_protect_until_ts:
                    self.log_message(
                        f"偏り是正(後処理): ロット {idx} は優先保護のためスキップ（出荷予定日: {shipping_date_raw}）",
                        debug=True,
                    )
                    continue
                ...
                for col in inspector_cols:
                    ...
                    if pd.isna(assigned_name) or str(assigned_name).strip() == '':
                        continue
                    assigned_clean = str(assigned_name).split('(')[0].strip()
                    if self._normalize_person_name(assigned_clean) != overloaded_norm:
                        continue
                    assigned_lots.append((normalized_ship_date, idx, col))
                    break

            assigned_lots.sort(key=lambda x: x[0], reverse=True)

            for _, lot_index, inspector_col in assigned_lots:
                if reassignment_count >= max_reassignments:
                    break

                row = result_df.loc[lot_index]
                product_number = row.get('品番', '')
                process_name = str(row.get('現在工程名', '') or '').strip()

                if self._is_locked_fixed_preinspection_lot(result_df, lot_index):
                    continue

                fixed_inspector_names = self._collect_fixed_inspector_names(product_number, process_name)
                if fixed_inspector_names:
                    self.log_message(
                        f"偏り是正(後処理): ロット {lot_index} は固定検査員設定のため対象外（品番: {product_number}）",
                        debug=True,
                    )
                    continue

                inspection_time = row.get('検査時間', 0.0)
                inspector_count = row.get('検査員人数', 1)
                try:
                    inspector_count = int(inspector_count) if pd.notna(inspector_count) else 1
                except Exception:
                    inspector_count = 1
                divided_time = inspection_time / inspector_count if inspector_count > 0 else 0.0
                ...
                if not available_inspectors:
                    self.log_message(
                        f"偏り是正(後処理): ロット {lot_index} は候補が0件のためスキップ（品番: {product_number}）",
                        debug=True,
                    )
                    continue

                lot_date_for_filter = self._resolve_lot_date(shipping_date, pd.Timestamp.now().date())
                available_inspectors = self.filter_available_inspectors(
                    available_inspectors,
                    divided_time,
                    inspector_master_df,
                    product_number,
                    relax_work_hours=False,
                    process_name_context=process_name,
                    ignore_product_limit=False,
                    lot_date=lot_date_for_filter,
                )
                if not available_inspectors:
                    self.log_message(
                        f"偏り是正(後処理): ロット {lot_index} はフィルタ後の候補が0件（品番: {product_number}）",
                        debug=True,
                    )
                    continue
                ...
                for col in inspector_cols:
                    ...
                    if not assigned_clean:
                        continue
                    info = self._get_inspector_by_name(assigned_clean, inspector_master_df)
                    if not info.empty:
                        current_codes.add(info.iloc[0]['#ID'])

                candidate_pool = [
                    insp for insp in available_inspectors
                    if _get_candidate_code(insp) not in current_codes
                ]
                if not candidate_pool:
                    self.log_message(
                        f"偏り是正(後処理): ロット {lot_index} は同ロット重複回避で候補なし（品番: {product_number}）",
                        debug=True,
                    )
                    continue

                candidate_pool.sort(key=lambda insp: self.inspector_work_hours.get(_get_candidate_code(insp), 0.0))

                replacement = None
                for insp in candidate_pool:
                    candidate_code = _get_candidate_code(insp)
                    if candidate_code not in under_loaded_set:
                        continue
                    candidate_hours = self.inspector_work_hours.get(candidate_code, 0.0)
                    overloaded_hours = self.inspector_work_hours.get(overloaded_code, 0.0)
                    if candidate_hours >= overloaded_hours:
                        continue
                    replacement = insp
                    break
                ...
                if replacement is None:
                    self.log_message(
                        f"偏り是正(後処理): ロット {lot_index} は余裕検査員に置換できずスキップ（品番: {product_number}）",
                        debug=True,
                    )
                    continue

                replacement_name = replacement['氏名']
                skill_value = replacement.get('スキル値', '')
                display_name = f"{replacement_name}({skill_value})" if skill_value else replacement_name
                result_df.at[lot_index, inspector_col] = display_name
                self.update_team_info(result_df, lot_index, inspector_master_df, show_skill_values)
                reassignment_count += 1
                self.log_message(
                    f"偏り是正(後処理): ロット {lot_index} の検査員を '{overloaded_name}' → '{replacement_name}' に変更しました",
                    debug=True,
                )
                self.log_message(
                    f"偏り是正(後処理): 品番 {product_number}, 出荷予定日 {shipping_date}, 列 {inspector_col}",
                    debug=True,
                )
        ...
        self.log_message(f"偏り是正(後処理): 再割当 {reassignment_count}件")
        return result_df

    def _run_post_final_fill_underutilized(
        self,
        result_df: pd.DataFrame,
        inspector_master_df: pd.DataFrame,
        skill_master_df: pd.DataFrame,
        ...
        10%超過は許容せず、偏りが悪化する割当は行わない。
        """
        self.log_message("全体最適化フェーズ2.7: 余裕検査員への追加割当を開始")
        ...
        try:
            ...
            self.log_message(f"追加割当の履歴再構築でエラーが発生しました: {e}", level='warning')
            return result_df
        ...
        # 現在の偏りを計算（全検査員を対象）
        all_codes = list(self.inspector_work_hours.keys())
        if not all_codes:
            return result_df
        current_max = max(self.inspector_work_hours.values())
        current_min = min(self.inspector_work_hours.values())
        current_imbalance = current_max - current_min

        # 未割当ロットを出荷予定日→指示日でFIFOソート
        df_sorted = result_df.copy()
        df_sorted['_ship_key_fill'] = df_sorted['出荷予定日'].apply(self._normalize_shipping_date) if '出荷予定日' in df_sorted.columns else pd.Timestamp.max
        if '指示日' in df_sorted.columns:
            df_sorted['_inst_key_fill'] = df_sorted['指示日'].apply(self._normalize_shipping_date)
        else:
            df_sorted['_inst_key_fill'] = pd.Timestamp.max
        df_sorted = df_sorted.sort_values(['_ship_key_fill', '_inst_key_fill', '品番'], ascending=[True, True, True], na_position='last')
        ...
        for idx, row in df_sorted.iterrows():
            ...
            inspection_time = row.get('検査時間', 0.0)
            if inspection_time is None or pd.isna(inspection_time) or inspection_time <= 0:
                continue

            required_inspectors = self._calc_required_inspectors(float(inspection_time))
            required_inspectors = max(1, min(MAX_INSPECTORS_PER_LOT, required_inspectors))
            divided_time = float(inspection_time) / required_inspectors
            ...
            if fixed_name_norms:
                available_inspectors = [
                    insp for insp in available_inspectors
                    if self._normalize_person_name(str(insp.get('氏名', '')).strip()) in fixed_name_norms
                ]
                if not available_inspectors:
                    self.log_message(
                        f"追加割当: 固定検査員が候補にいないためスキップ（品番: {product_number}）",
                        debug=True,
                    )
                    continue
            ...
            for threshold in thresholds:
                underutilized = []
                for insp in available_inspectors:
                    code = insp.get('コード', insp.get('#ID', insp.get('コーチID', insp.get('コーチE', ''))))
                    if not code:
                        continue
                    max_hours = self.get_inspector_max_hours(code, inspector_master_df)
                    allowed_max_hours = self._apply_work_hours_overrun(max_hours)
                    daily_hours = self.inspector_daily_assignments.get(code, {}).get(current_date, 0.0)
                    if daily_hours < allowed_max_hours * threshold:
                        underutilized.append(insp)

                if not underutilized:
                    continue

                underutilized.sort(
                    key=lambda insp: self.inspector_work_hours.get(
                        insp.get('コード', insp.get('#ID', insp.get('コーチID', insp.get('コーチE', '')))),
                        0.0,
                    )
                )
                candidate = self.select_inspectors_with_skill_combination(
                    underutilized,
                    required_inspectors,
                    divided_time,
                    pd.Timestamp.now(),
                ...
                projected_hours = self.inspector_work_hours.copy()
                for insp in candidate:
                    code = insp.get('コード', insp.get('#ID', insp.get('コーチID', insp.get('コーチE', ''))))
                    if not code:
                        continue
                    projected_hours[code] = projected_hours.get(code, 0.0) + divided_time

                new_max = max(projected_hours.values())
                new_min = min(projected_hours.values())
                new_imbalance = new_max - new_min
                if new_imbalance > current_imbalance + 0.2:
                    continue
            ...
            # 割当を反映
            for i in range(1, MAX_INSPECTORS_PER_LOT + 1):
                result_df.at[idx, f'検査員{i}'] = ''
            for i, insp in enumerate(selected, start=1):
                name = insp.get('氏名', '')
                skill_value = insp.get('スキル値', '')
                display_name = f"{name}({skill_value})" if skill_value else name
                if i <= MAX_INSPECTORS_PER_LOT:
                    result_df.at[idx, f'検査員{i}'] = display_name
            result_df.at[idx, '検査員人数'] = len(selected)
            result_df.at[idx, '分割検査時間'] = round(divided_time, 1)
            self.update_team_info(result_df, idx, inspector_master_df, show_skill_values)
        ...
        # 追加で使用した列を削除
        result_df.drop(columns=['_ship_key_fill', '_inst_key_fill'], errors='ignore', inplace=True)
        self.log_message(f"追加割当: {additional_assigned}件")
        return result_df

    def _log_exception_lot_summary(self, result_df: pd.DataFrame) -> None:
        if result_df is None or result_df.empty or '出荷予定日' not in result_df.columns:
            return

        shipping_dates = result_df['出荷予定日']
        pre_mask = shipping_dates.apply(self._is_preinspection_label)
        same_day_mask = shipping_dates.apply(self._is_same_day_cleaning_label)
        inspector_counts = pd.to_numeric(result_df.get('検査員人数', 0), errors='coerce').fillna(0)
        unassigned_mask = inspector_counts <= 0

        def summarize(mask: pd.Series, label: str) -> None:
            if not mask.any():
                return
            total = int(mask.sum())
            assigned = int((~unassigned_mask & mask).sum())
            unassigned = total - assigned
            self.log_message(f"例外ロット内訳: {label} {total}件 (割当 {assigned} / 未割当 {unassigned})")
            if unassigned > 0 and 'チーム情報' in result_df.columns:
                reasons = (
                    result_df.loc[mask & unassigned_mask, 'チーム情報']
                    .fillna('未割当')
                    .astype(str)
                    .value_counts()
                    .head(3)
                ...
                for reason, count in reasons.items():
                    self.log_message(f"  - {label} 未割当理由: {reason} ({count}件)")

        summarize(pre_mask, "先行検査")
        summarize(same_day_mask & ~pre_mask, "当日洗浄")

    def _log_utilization_summary(
        self,
        result_df: pd.DataFrame,
        inspector_master_df: pd.DataFrame,
        skill_master_df: pd.DataFrame,
        ...
        if result_df is None or result_df.empty or not self.inspector_work_hours:
            return
        ...
        for code, hours in self.inspector_work_hours.items():
            ...
            if allowed_max <= 0:
                continue
            utilization = hours / allowed_max
            if utilization < 0.9:
                name = name_map.get(str(code).strip(), str(code).strip())
                under_90.append((utilization, name, hours, allowed_max))

        if not under_90:
            self.log_message("勤務時間稼働率: 90%未満 0名")
            return

        under_90.sort(key=lambda x: x[0])
        self.log_message(f"勤務時間稼働率: 90%未満 {len(under_90)}名")
        for utilization, name, hours, allowed_max in under_90[:10]:
            self.log_message(
                f"  - {name}: {hours:.1f}h / {allowed_max:.1f}h ({utilization * 100:.1f}%)"
            )

        # 診断: 90%未満者が未割当ロットに対して候補になれるか
        unassigned_df = result_df.copy()
        if '検査員人数' in unassigned_df.columns:
            inspector_counts = pd.to_numeric(unassigned_df['検査員人数'], errors='coerce').fillna(0).astype(int)
            unassigned_df = unassigned_df[inspector_counts <= 0]
        if unassigned_df.empty or '検査時間' not in unassigned_df.columns:
            return

        unassigned_df = unassigned_df[pd.to_numeric(unassigned_df['検査時間'], errors='coerce').fillna(0.0) > 0]
        ...
        self.log_message("勤務時間稼働率: 90%未満者の候補ロット診断", debug=True)
        current_date = pd.Timestamp.now().date()
        unassigned_divided_times = []
        for _, row in unassigned_df.iterrows():
            inspection_time = row.get('検査時間', 0.0)
            try:
                inspection_time = float(inspection_time)
            except Exception:
                continue
            required_inspectors = self._calc_required_inspectors(inspection_time)
            required_inspectors = max(1, min(MAX_INSPECTORS_PER_LOT, required_inspectors))
            divided_time = inspection_time / required_inspectors if required_inspectors > 0 else 0.0
            if divided_time > 0:
                unassigned_divided_times.append(divided_time)
        if unassigned_divided_times:
            divided_series = pd.Series(unassigned_divided_times)
            self.log_message(
                "未割当ロット分割時間: "
                f"最小 {divided_series.min():.2f}h / 中央 {divided_series.median():.2f}h / 最大 {divided_series.max():.2f}h",
                debug=True,
            )

        def _diagnose_exclusion_reason(
            available_inspectors: List[Dict[str, Any]],
            inspector_code: str,
            divided_time: float,
            product_number: str,
            ...
            target = next(
                (insp for insp in available_inspectors if _candidate_code(insp) == inspector_code),
                None,
            )
            if not target:
                return "候補に非含有"

            inspector_name = str(target.get('氏名', '') or '').strip()
            vacation_info = self.get_vacation_info(inspector_name)
            if vacation_info:
                code = vacation_info.get("code", "")
                if code in ["休", "出", "当"]:
        ...
        for utilization, name, hours, allowed_max in under_90[:10]:
            inspector_info = self._get_inspector_by_name(name, inspector_master_df)
            if inspector_info.empty:
                continue
            inspector_code = str(inspector_info.iloc[0].get('#ID', inspector_info.iloc[0].get('#コード', ''))).strip()
            if not inspector_code:
                continue
            candidate_count = 0
            normalized_name = self._normalize_person_name(name)
            remaining_hours = allowed_max - hours
            remaining_time_total = 0
            remaining_time_candidate = 0
            reason_counts = {
                "固定検査員外": 0,
                "スキル/工程候補なし": 0,
                "候補に非含有": 0,
            ...
            for _, row in unassigned_df.iterrows():
                ...
                process_name = str(row.get('現在工程名', '') or '').strip()
                shipping_date = row.get('出荷予定日', None)
                if divided_time <= remaining_hours:
                    remaining_time_total += 1
                ...
                if fixed_inspector_names:
                    ...
                    if normalized_name not in fixed_name_set:
                        reason_counts["固定検査員外"] += 1
                        continue
                ...
                if not available_inspectors:
                    reason_counts["スキル/工程候補なし"] += 1
                    continue
                if not any(_candidate_code(insp) == inspector_code for insp in available_inspectors):
                    reason_counts["候補に非含有"] += 1
                    continue
                if divided_time <= remaining_hours:
                    remaining_time_candidate += 1

                base_available_inspectors = available_inspectors
                lot_date_for_filter = self._resolve_lot_date(shipping_date, pd.Timestamp.now().date())
                available_inspectors = self.filter_available_inspectors(
                    available_inspectors,
                    divided_time,
                ...
                if not available_inspectors:
                    reason = _diagnose_exclusion_reason(
                        base_available_inspectors,
                        inspector_code,
                        divided_time,
                        product_number,
                    )
                ...
                if any(_candidate_code(insp) == inspector_code for insp in available_inspectors):
                    candidate_count += 1
                else:
                    reason = _diagnose_exclusion_reason(
                        base_available_inspectors,
            ...
            self.log_message(f"  - {name}: 候補ロット {candidate_count}件", debug=True)
            self.log_message(
                f"    残り時間 {remaining_hours:.1f}h / 残り時間内ロット {remaining_time_total}件 / "
                f"候補含有 {remaining_time_candidate}件",
                debug=True,
            )
            top_reasons = [
                (reason, count)
