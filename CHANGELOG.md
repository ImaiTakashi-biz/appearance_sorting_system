...
## v1.2.9 (2026-02-11)
- Step1: 割当ロジックの品質改善を継続実施（当日洗浄・先行検査・直近出荷ロットの優先度/FIFO整合、未割当再処理、勤務時間超過抑止、偏り是正）。
- Step1: 先行検査品の割当人数ルールと分散挙動を調整し、過剰分割・重複割当の発生を抑制。
- Step1: 同一検査員への類似品番重複割当制約（08131-00810 と 08131-01010）を追加。
- Step1: 当日洗浄ロットで通常スキル担当が不在時、新製品対応チームへのフォールバック割当を追加。
- Step2: UIログの既定設定を軽量化（割当差分デバッグログは既定OFF）し、重複ログ自動抑止（閾値超過時サプレッション）を追加。
- Step2: 不要な復旧/一時ファイルを整理して運用時のノイズを削減。

## v1.2.8 (2026-01-28)
- 割当ロジック: 優先ロット（当日出荷/先行検査/当日洗浄）を最終是正でも未割当にしない保護を追加し、FIFO厳守後の再割当でも優先ロットを保持（`app/assignment/inspector_assignment_service.py`）
- 割当ロジック: 最終表示KPIの前に割当履歴を再構築し、ログとUIの整合性を改善。稼働率ログは結果テーブル（検査員列）から再集計して一致性を強化
- 偏り是正: 低稼働救済の再割当回数上限を緩和し、過去実績・同工程実績に基づく候補補完を追加（優先ロット/固定検査員/勤務時間上限は維持）
- UI: 先行検査品の割当人数変更は「今回のみ反映」とし、登録済み品番リストは保存しない仕様を維持
- UI: 検査員名の表示/座席表の割当生成で空欄を正しく除外（`nan`誤認識の抑止）し、ログと表示の不整合を改善
- UI性能: 検査員割振りテーブルの描画を高速化（列幅キャッシュ、日付/検査員表示の事前計算、行ループの軽量化）
- ログ負荷: 署名/差分スナップショットのデフォルトを無効化し、UIデバッグログは環境変数で有効化する方式に変更

## v1.2.7 (2026-01-26)
- 改善: 検査員割当ロジックを強化し、未使用のロットに対する割当機能を改善（`app/assignment/inspector_assignment_service.py`）
  - 検査員の稼働率（`daily_utilization`）を考慮した割当処理を実装
  - 未使用検査員を優先的に割り当てるロジックを追加
  - 優先順位を調整：固定検査員 > 未使用検査員 > 稼働率が低い順 > 総作業時間が少ない順
  - 低稼働率検査員への再割当処理を強化
- 追加: 検査対象CSVのキーワードによるフィルタリング機能（`app/ui/ui_handlers.py`）
  - `_filter_lots_by_inspection_keywords()`メソッドを実装し、工程名をキーワードで絞り込み可能に
  - 不足品番側、登録済み品番、洗浄二次処理依頼のロット取得時に適用
  - 正規表現による部分一致検索（大文字小文字を区別しない）
- 改善: UIの文言を日本語に統一し、ユーザー体験を向上（`app/ui/ui_handlers.py`）

## v1.2.6 (2026-01-23)
- UI: 先行検査品/登録済み品番の曜日を複数選択（平日）に対応し、土日を削除、曜日表示幅を拡大。(`app/ui/ui_handlers.py`)
- UI: 追加割当ダイアログで品番の直接入力に製品マスタ候補と固定検査員選択を追加し、品番ごとの上書き設定（ロット数/工程/号機/割当人数/固定検査員）、マウスホイールスクロール、登録値の可視化に対応。上書きは追加割当時のみ適用。(`app/ui/ui_handlers.py`)
- UI: 追加割当ダイアログのリスト表示を高さ固定のフレームにまとめ、ダイアログ高さを拡大して下部ボタンを常時表示。登録済み品番/候補ドロップダウンのスクロール速度と挙動も調整。(`app/ui/ui_handlers.py`)
- UI: 追加割当設定ダイアログに固定検査員を、検査員選択ダイアログでチェックボックスから選べるボタンを追加。(`app/ui/ui_handlers.py`)
