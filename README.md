...
現在のバージョン: `v1.2.8`（`version.py` を参照）

このプロジェクトは Access の出荷検査残データとマスタ情報を組み合わせ、検査員のスキル・休暇・固定アサイン情報を考慮して公平な割当を自動化するWindows向けPythonアプリです。

## 最近の更新（v1.2.8 / 2026-01-28）
- 優先ロット（当日出荷/先行検査/当日洗浄）の割当を再調整しても未割当にならないよう保護し、FIFO順に再割当しても優先ロットを保持するロジックを導入しました。
- 稼働率ログを割当結果テーブルから再集計するようにしてログとUIの整合性を改善し、低稼働救済の再割当回数制限を緩和しつつ候補補完を強化しました。
- UIでは検査員名や座席表の空欄排除、追加割当の表示高速化、先行検査品の割当人数変更の一時適用、ログ出力のデフォルト抑止／環境変数による有効化などにより操作性と診断性を向上しています。
