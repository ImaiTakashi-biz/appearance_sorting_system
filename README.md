...
現在のバージョン: `v1.2.9`（`version.py` を参照）

このプロジェクトは Access の出荷検査残データとマスタ情報を組み合わせ、検査員のスキル・休暇・固定アサイン情報を考慮して公平な割当を自動化するWindows向けPythonアプリです。

## 最近の更新（v1.2.9 / 2026-02-11）
- Step1（割当ロジック品質改善）: 当日洗浄・先行検査・直近出荷ロットの優先割当/FIFOの安定化、勤務時間超過抑止、未割当再処理と配分バランスの改善を実施しました。
- Step2（性能・UI体感改善）: 割当差分デバッグログを既定で抑止し、重複ログの自動サプレッションを追加して、実行時ログ量とUI体感の悪化を抑制しました。
- 保守性改善: 不要な一時/復旧ファイルを整理し、運用時のノイズを削減しました。
